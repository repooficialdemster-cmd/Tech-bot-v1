...
let handler = async (m, { conn, args, command, usedPrefix }) => {
  ...
  try {
    ...
    // 🔑 Usar TU API (Adonix)
    let apiUrl = `https://myapiadonix.vercel.app/download/yt?url=${encodeURIComponent(url)}&format=${isAudio ? 'mp3' : 'mp4'}`
    let res = await fetch(apiUrl)
    if (!res.ok) throw new Error('Error al conectar con la API.')
    let json = await res.json()
    if (!json.status || !json.result?.download) throw new Error('No se pudo obtener descarga.')
