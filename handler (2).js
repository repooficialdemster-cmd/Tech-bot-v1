...
const randomDelay = Math.floor(Math.random() * 3000) + 1000; // 1-3 segundos
await this.readMessages([m.key]); // Marcar como visto
await this.sendPresenceUpdate('composing', m.chat); // Aparecer como escribiendo
await new Promise(resolve => setTimeout(resolve, randomDelay)); // Pausa aleatoria
await this.sendPresenceUpdate('paused', m.chat); // Dejar de escribir
